...
public class PostController {
    ...
    @ResponseBody
    public void getPostid(Integer postid,HttpServletRequest request, HttpServletResponse response)throws Exception{
        response.setContentType("text/html;charset=utf-8");
    ...
    @ResponseBody
    public  MyMessage postDetail(Integer postid,HttpServletRequest request, HttpServletResponse response)throws Exception{
        response.setContentType("text/html;charset=utf-8");
    ...
    public void removePost(Integer jn_postid,HttpServletResponse response)throws Exception
    {
        ...
        if (postService.removePostByPostid(jn_postid))
        {
            ...
            {
                out.print("<script>alert('删除失败请重新！');location.href='personal.jsp';</script>");
            }
        out.flush();
        out.close();
    }
    @RequestMapping("/updatePost")
    public String updatePost(Post jn_post,HttpSession session)
    {
       session.setAttribute("mypostlm",jn_post);
        return "updatepost";
    }
    @RequestMapping("/updatePost2")
    public String updatePost2(MultipartFile jn_logo, String posttitle, String[] posttype, String postcontent, HttpServletRequest request, HttpServletResponse response)throws Exception
    {
        response.setContentType("text/html;charset=utf-8");
        PrintWriter pw=response.getWriter();
        Post post1=new Post();
        post1.setJn_postcontent(postcontent);
        post1.setJn_posttitle(posttitle);
        //获取当前服务器磁盘地址
        String path=  request.getServletContext().getRealPath("/upload");
        //判断当前文件夹是否存在,不存在则创建
        File file =new File(path);
        ...
        post1.setJn_logo(filename);
        LocalDateTime time=LocalDateTime.now();
        String posttime=time.toString();
        posttime=posttime.substring(0,10)+" "+posttime.substring(11,16);
        post1.setJn_posttime(posttime);
        boolean flag=postService.updatePostByPost(post1);
        Postidvo postidvo =postService.quaryPostidByUseridAndPosttime(post1.getJn_postid(),posttime);
        if (flag){
            for(int i=0;i<posttype.length;i++){
                String type=posttype[i];
                System.out.println(post1.getJn_postid());
                Posttype poty =posttypeService.quarryPostidByPosttypename(type);
                boolean flag1=post_posttypeService.updatePostTypeByPostId(postidvo.getJn_postid(),poty.getJn_posttypeid());
                if (flag1)
                {
                    pw.print("<script>alert('修改成功');location.href='personal.jsp';</script>");
                }
                else
                {
                    pw.print("<script>alert('类型添加错误');location.href='editpost.jsp';</script>");
                }
            }

        }else{
            pw.print("<script>alert('修改失败');location.href='editpost.jsp';</script>");
        }
        pw.flush();
        pw.close();
        return "personal";
